...
from kerckhoff.packages.operations.s3_utils import get_s3_client, upload_file, get_bucket_region
...
class ImageUtils:
    ...
    def snapshot_images(self, google_drive_image_files, bucket, package_versions, quality=95):
            ...
            if res.ok:
                ...
                with tempfile.NamedTemporaryFile(suffix='.jpg', delete=False) as f:
                    ...
                    for chunk in res:
                        f.write(chunk)
                    f.flush()
                    self._compress_image(image_path, quality=quality, mimetype=drive_image_file.mimeType)
                    key = upload_file(s3, image_path, bucket, drive_image_file.mimeType)
                    self._save_item(package_versions, key, bucket, get_bucket_region(s3, bucket),
    ...
    def _save_item(self, package_versions, key, bucket, region, mimetype, meta=None):
        ...
        item = PackageItem(data_type=PackageItem.IMAGE,
                           data=data,
                           file_name=key,
                           mime_types=mimetype)
        item.save()
